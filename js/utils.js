        ...
        if(msg instanceof Array)
        {
            for(var i=0;i<msg.length;i++)
            {
                if(msg[i].trim().length > 0)
                    resWin.append("<p class=\"text-"+type+" bold\">"+msg[i]+"</p>");
            }
        }
        else
        {
            resWin.append("<p class=\"text-"+type+" bold\">"+msg+"</p>");
        }
    }
    resWin.scrollTop(resWin[0].scrollHeight);
}

/*
 * Check if there are unsaved content in Code Editor
 */
function checkForUnsaved()
{
    // Return true if user want to continue
    // Return false if user want to cancel reloading
    if(editChanged)
    {
        return confirm("You have unsaved file. Do you still want to continue?");
    }
    else return true;
}
...
/*
 * Recollapse a file tree
 */
function recollapse(filepath)
{
    var dirpath = pathinfo(filepath,1);
    dirpath += "/";
    var m = md5(dirpath);
    $('#'+m).click();
    $('#'+m).click();
